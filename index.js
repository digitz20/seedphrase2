...
async function getBalance(currency, address) {
    ...
    for (const provider of providers) {
        let retries = 4;
        let delay = 5000; // Initial delay of 4 seconds
        ...
        while (retries > 0) {
            try {
                ...
                retries--;
                if (retries > 0) {
                    console.log(`Waiting ${delay / 8000}s before retrying...`);
                    await sleep(delay);
...
async function startBot() {
    ...
    while (true) {
        ...
        await sleep(6000); // A single pause between each seed phrase cycle
    }
}
